...
public class ExtremePoints3D<P extends Placement3D & Serializable> implements ExtremePoints<P, Point3D<P>> {
    ...
    protected void addFirstPoint() {
        values.add(new Default3DPlanePoint3D<>(0, 0, 0, containerMaxX, containerMaxY, containerMaxZ, containerPlacement,
    ...
    public boolean add(int index, P placement) {
        ...
        /*
        总体方法: 不要迭代放置以查找最大mins点，而是投影现有点。
        投影被放置所吞噬的点，然后删除它们被放置所遮蔽的项目点到另一侧添加被两个新点 (如果它们可以在负方向上移动)
         遮蔽的点删除被其他人遮蔽的点尽可能跟踪放置边界
         */
        ...
        if (!moveToXX.isEmpty()) {
            ...
            for (int i = 0; i < moveToXX.size(); i++) {
                ...
                // TODO skip x
                while (targetIndex < values.size()
                    && Point3D.COMPARATOR_X_THEN_Y_THEN_Z.compare(added, values.get(targetIndex)) > 0) {
                    targetIndex++;
        ...
        if (supported) {
            // not necessary
        } else if ((supportedXYPlane && supportedXZPlane) || (supportedXYPlane && supportedYZPlane)
        ...
        int added = addedXX.size() + addedYY.size() + addedZZ.size() + constrainXX.size() + constrainYY.size()
            + constrainZZ.size();
                    ...
                    if (point.getMinX() < placement.getAbsoluteX()) {
                        if (point.getMaxX() >= placement.getAbsoluteX()) {
                            ...
                            if (area >= minAreaLimit) {
                                Point3D<P> clone =
                                    point.clone(placement.getAbsoluteX() - 1, point.getMaxY(), point.getMaxZ());
                ...
                if (withinZ(point.getMinZ(), placement)) {
                    if (point.getMinY() < placement.getAbsoluteY()) {
                        if (point.getMaxY() >= placement.getAbsoluteY()) {
                            ...
                            if (area >= minAreaLimit) {
                                Point3D<P> clone =
                                    point.clone(point.getMaxX(), placement.getAbsoluteY() - 1, point.getMaxZ());
    ...
    protected void constrainFloatingMaxWithClone(P placement, int limit) {
        ...
        */
        ...
        for (int i = 0; i < limit; i++) {
            ...
            if (placement.getAbsoluteEndX() < point.getMinX() || placement.getAbsoluteEndY() < point.getMinY()
                || placement.getAbsoluteEndZ() < point.getMinZ() || placement.getAbsoluteX() > point.getMaxX()
                || placement.getAbsoluteY() > point.getMaxY() || placement.getAbsoluteZ() > point.getMaxZ()) {
                continue;
            }
    ...
    protected void constrainFloatingMax(P placement, int limit) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            if (placement.getAbsoluteEndZ() < point.getMinZ() || placement.getAbsoluteEndY() < point.getMinY()
                || placement.getAbsoluteEndX() < point.getMinX() || placement.getAbsoluteX() > point.getMaxX()
                || placement.getAbsoluteY() > point.getMaxY() || placement.getAbsoluteZ() > point.getMaxZ()) {
                continue;
            }
