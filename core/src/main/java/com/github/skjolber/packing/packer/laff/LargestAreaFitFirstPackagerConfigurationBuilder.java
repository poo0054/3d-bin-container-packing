...
public abstract class LargestAreaFitFirstPackagerConfigurationBuilder<P extends Point2D<StackPlacement>,
    B extends LargestAreaFitFirstPackagerConfigurationBuilder<P, B>> {
    ...
    public static StackValuePointFilter DEFAULT_STACK_VALUE_POINT_FILTER =
        (stackable1, point1, stackValue1, stackable2, point2, stackValue2) -> {
            if (stackable2.getVolume() == stackable1.getVolume()) {
                if (stackValue1.getArea() == stackValue2.getArea()) {
                    // closest distance to a wall is better 离墙最近的距离更好

                    int distance1 =
                        Math.min(point1.getDx() - stackValue1.getDx(), point1.getDy() - stackValue1.getDy());
                    int distance2 =
                        Math.min(point2.getDx() - stackValue2.getDx(), point2.getDy() - stackValue2.getDy());
    ...
    public static StackValuePointFilter FIRST_STACK_VALUE_POINT_FILTER =
        (stackable1, point1, stackValue1, stackable2, point2, stackValue2) -> {
            if (stackValue1.getArea() == stackValue2.getArea()) {
    ...
    public LargestAreaFitFirstPackagerConfiguration<P> build() {
        ...
        return new DefaultLargestAreaFitFirstPackagerConfiguration<P>(firstStackableComparator,
            firstStackValuePointComparator, nextStackableComparator, nextStackValuePointComparator);
    }
